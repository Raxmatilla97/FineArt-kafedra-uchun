...
trait HasLegacyDefinitions
{
        ...
        if (!$scope->modelScope && ($scopeName = $scope->scope)) {
            $scope->modelScope = function($query, $scope) use ($scopeName) {
                return $query->$scopeName(
                    $this->refitLegacyDateParse($scope->scopeValue !== null ? $scope->value : $scope->default)
                );
            };
        }
    }
    ...
    protected function refitLegacyDateRangeScope($scope)
    {
        ...
        if (!$scope->modelScope && ($scopeName = $scope->scope)) {
            $scope->modelScope = function($query, $scope) use ($scopeName) {
                return $query->$scopeName(
                    $this->refitLegacyDateParse($scope->scopeValue !== null ? $scope->after : $scope->defaultAfter),
                    $this->refitLegacyDateParse($scope->scopeValue !== null ? $scope->before : $scope->defaultBefore)
                );
            };
        }
    }
    ...
    protected function refitLegacyNumberScope($scope)
    {
        ...
        if (!$scope->modelScope && ($scopeName = $scope->scope)) {
            $scope->modelScope = function($query, $scope) use ($scopeName) {
                return $query->$scopeName($scope->scopeValue !== null ? $scope->value : $scope->default);
            };
        }
    }
    ...
    protected function refitLegacyNumberRangeScope($scope)
    {
        ...
        if (!$scope->modelScope && ($scopeName = $scope->scope)) {
            $scope->modelScope = function($query, $scope) use ($scopeName) {
                return $query->$scopeName(
                    $scope->scopeValue !== null ? $scope->min : $scope->defaultMin,
                    $scope->scopeValue !== null ? $scope->max : $scope->defaultMax
                );
            };
        }
    }
